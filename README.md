...
```
band-of-agents/
├── material_theory_agent/          # Agent 1 — Material Theory Agent
│   ├── main.py                     # CLI entry point (band server or direct mode)
│   ├── graph.py                    # LangGraph workflow definition
│   ├── band_graph.py               # Band platform wiring + ReAct orchestrator
│   ├── state.py                    # LangGraph typed state schema
│   ├── custom_preprocessor.py      # Band message preprocessor
│   ├── requirements.txt            # Python dependencies
│   ├── .env.example                # Environment variable template
│   ├── nodes/                      # LangGraph pipeline nodes
│   │   ├── requirement_parser.py   # Node 1: NL → structured spec
│   │   ├── theory_planner.py       # Node 2: scientific strategy
│   │   ├── scientific_reasoner.py  # Node 3: hypothesis generation
│   │   ├── material_generator.py   # Node 4: candidate material
│   │   └── constraint_checker.py  # Node 5: validation + PASS/FAIL routing
│   ├── tools/
│   │   └── scientific_tools.py     # PubChem, Wikipedia, periodic table, cost
│   └── utils/
│       ├── llm.py                  # Rate-limit-aware Groq LLM wrapper
│       └── json_parser.py          # Safe JSON parsing utility
│
├── virtual_lab_agent/              # Agent 2 — Virtual Lab Agent
│   ├── virtual_lab_agent.py        # Band platform wiring (CrewAIAdapter)
│   ├── pyproject.toml              # uv project config
│   ├── knowledge/
│   │   └── user_preference.txt     # CrewAI knowledge context
│   └── src/agent_2v02/
│       ├── crew.py                 # CrewAI crew definition (2 agents, 2 tasks)
│       ├── main.py                 # Standalone runner
│       ├── models.py               # VirtualLabReport Pydantic schema
│       ├── config/
│       │   ├── agents.yaml         # Agent role & backstory definitions
│       │   └── tasks.yaml          # Task definitions
│       └── tools/
│           └── custom_tool.py      # ManufacturingCostCalculatorTool (deterministic)
│
└── procurement_and_sourcing_agent/ # Agent 3 — Procurement & Sourcing Agent
    ├── procurement_agent.py        # Band platform wiring (PydanticAIAdapter)
    ├── pyproject.toml              # uv project config
    ├── .env.example                # Environment variable template
    ├── agent_config.yaml.example   # Band credentials template
    └── src/agent_3v01/
        ├── pipeline.py             # pydantic-ai agent definition
        ├── main.py                 # Standalone runner
        ├── models.py               # ExecutiveProductProposal Pydantic schema
        └── tools/
            └── sourcing_tools.py   # lookup_supply_chain + compute_roi (deterministic)
```
